  ...
  class ReduceCopier<K, V> implements MRConstants {
    ...
    private class MapOutputCopier extends Thread {
      ...
      private long copyOutput(MapOutputLocation loc
        ...
        LOG.info("%%%%%%%%%%%%%%%%%%%%%%%%%% REDUCE going to copy output %%%%%%%%%%%%%%%%%%%%%%%%%5");
        LOG.info("Current Location : "+this.currentLocation.getHost());
      ...
      throws IOException, InterruptedException {
    	  boolean localCopy = conf.getBoolean("localcopy", false);
    	  MapOutput mapOutput = null;
    	  LOG.info("LOCAL COPY IS : "+localCopy);
    	  if(localCopy && "192.168.140.96".equals(InetAddress.getByName(mapOutputLoc.getHost()).getHostAddress())){
          	LOG.info("Getting from local output");
          	mapOutput = CopyFileLocally(mapOutputLoc, filename, reduce);
          	return mapOutput;
          } 
      ...
      private MapOutput CopyFileLocally(MapOutputLocation mapOutputLoc, 
              Path filename, int reduce) throws IOException{
    	  URL url = mapOutputLoc.getOutputLocation();
    	  MapOutput mapOutput =null;
    	  
    	  // getting the jobId
    	  String jobId = new String(url.getFile().getBytes()).split("=")[1].split("&")[0];
    	  String pathStr = conf.get("hadoop.tmp.dir","/data/hadoop-0.20.204.0/tmp");

    	  // path of the map output file
          pathStr+="/mapred/local/taskTracker/tunga/jobcache/"+jobId+"/"+mapOutputLoc.getTaskAttemptId()+"/output";

          // get the proper index for the sequence file
          IndexCache indexCache = new IndexCache(conf);
          Path indexFilePath = new Path(pathStr+"/file.out.index");
          IndexRecord indexRecord = indexCache.getIndexInformation(mapOutputLoc.getTaskAttemptId().toString(), reduce, indexFilePath, "tunga");
          Path localFilename = 
                  lDirAlloc.getLocalPathForWrite(filename.toUri().getPath(),conf);
        
          // open map output file
          String mapLocalFileName = pathStr+"/file.out";
          FileInputStream mapOutputIn = SecureIOUtils.openForRead(
                  new File(mapLocalFileName), "tunga");
          
          // skip to the offset for this reducer.
          mapOutputIn.skip(indexRecord.startOffset);
        
          
          // copy from local inputStream to local outputStream
          OutputStream outputStream = null;
          int MAX_BYTES_TO_READ = 64 * 1024;
          byte[] buffer = new byte[MAX_BYTES_TO_READ];
          long bytesRead = 0;
          try {
            outputStream = rfs.create(localFilename);
            long rem = indexRecord.partLength;
            int len =
            ...
            while (rem > 0 && len >= 0) {
              ...
              len = mapOutputIn.read(buffer, 0, (int)Math.min(rem, MAX_BYTES_TO_READ));
               bytesRead += len;
              shuffleClientMetrics.inputBytes(len);
      ...
      throws IOException {
        ...
        LOG.info("@@@@ @@@@ @@@@ ::::: Writing mapout to : "+localFilename.toUri().getPath());
        MapOutput mapOutput = 
          new MapOutput(mapOutputLoc.getTaskId(), mapOutputLoc.getTaskAttemptId(), 
                        conf, localFileSys.makeQualified(localFilename), 
                        mapOutputLength);
